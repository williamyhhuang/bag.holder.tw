...
【最小持倉天數原則】
本策略設計最佳持倉周期為 21 天以上。
- holding_days < 21：訊號極可能是短期雜訊，除非 MACD Death Cross 且明顯虧損（pnl_pct < -5%），否則傾向 hold 或 watch
- holding_days 21-60：正常出場判斷期，依技術面與基本面綜合評估
- holding_days > 60：逾期持倉，即使是軟性訊號（RSI Momentum Loss）也應認真考慮出場

分類標準：
- sell：訊號強烈（MACD Death Cross 或 Death Cross）且族群轉弱或財報衰退，建議立即出場；或 holding_days < 21 但已明顯虧損（pnl_pct < -5%）
- watch：RSI Momentum Loss 等軟性訊號，或財務/族群仍可，建議設停損繼續觀察；或 holding_days < 21 的一般賣出訊號
- hold：訊號偏弱且族群仍強、財報成長，可能是假突破；或 holding_days < 21 的輕微訊號
