...
權重依 Phase 2 IC 驗證結果調整（2022-2026，1981 支股票）：
  vol_ratio: IC=0.0144, t=3.77, IC>0率 63% → 唯一通過，提升至 50%
  rps_6m:    IC=0.0100, t=1.33, IC>0率 56% → 方向正確，維持 25%
  rps_3m:    IC=0.0070, t=0.89, IC>0率 56% → 較弱，降至 15%
  inst:      無歷史資料驗證，保守降至 10%

綜合分數 = 量能 * 0.50 + RPS_6m * 0.25 + RPS_3m * 0.15 + 法人 * 0.10
"""
...
# 因子權重（加總 = 1.0）
# 注意：IC 驗證（Phase 2）是對全市場截面計算，而因子排名作用於候選池截面。
# vol_ratio 在全市場 IC 顯著，但候選股已由技術訊號完成量能篩選，
# 在候選池中 RPS 的區分度更高，故維持原始權重。
_WEIGHT_RPS_3M = 0.25
_WEIGHT_RPS_6M = 0.25
